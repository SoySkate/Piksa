...
export default function Home() {
  ...
  //Supuestamente hago lo mimso en DataContext pero como la funcion de AuthContext 
  ...
  const[textV, setTextV] = useState<string>('');
     ...
     if(loginUser.nombre && loginUser.password !== ''){
       ...
       userList.map(e=>{
        ...
        if(e.nombre===loginUser.nombre&&e.password === loginUser.password){
            ...
            //Me entra en el else tambien por la cara xd why?
        }else{setAlert('Usuario o contraseña incorrectas.'); setTextV('FALSE');console.log('aqui NO entras en el login pq NOcoinciden los datos')}
       })
     }
     console.log(loginUser);
   };
   //ButtonRegisterPageFunction
   const registerPage=()=>{
  ...
  <main className='overflow-hidden w-screen h-screen flex justify-evenly items-center'>
    ...
    <div>      
      <div className='flex flex-col items-center space-y-8 mt-8 '>
        ...
        <div> <button className='bg-blue-400 rounded-2xl px-4 py-1 my-2 font-mono font-semibold' onClick={registerPage}>Registrarse</button></div>
